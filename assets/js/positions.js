...
// Front-Matter laden das hier überhaupt. Die Basis-Grafik (Feld + graue
// Positions-Punkte) kommt fertig aus dem Server-Build (lib/formation-diagram.js);
// dieses Skript ändert nur Klassen/Textinhalte, es zeichnet nichts neu.
document.addEventListener("DOMContentLoaded", () => {
  const widget = document.querySelector(".positions-widget");
  if (!widget) return;

  // ---------- Tabs ----------
  const tabs = widget.querySelectorAll(".positions-tab");
  const panels = widget.querySelectorAll(".positions-panel");
  ...
  tabs.forEach((tab) => {
    tab.addEventListener("click", () => {
      ...
      panels.forEach((p) => {
        p.hidden = p.dataset.panel !== tab.dataset.target;
      });
    });
  });

  // ---------- Positions-Punkte je Panel ----------
  panels.forEach((panel) => {
    ...
    // Verlassen wieder. lockedId: bleibt bestehen (Klick/Tap/Tastatur-
    // Fokus), damit Touch-Geräte ohne echtes Hover trotzdem eine
    // dauerhafte Auswahl bekommen.
    let hoverId = null;
    let lockedId = null;
