...
// umrandeter Kreis mit dem Kürzel mittig als Beschriftung — filled=true
// (durchgezogene Füllung) für ein Team, filled=false (nur Kontur, Rasen
// scheint durch) fürs andere. So lassen sich zwei Teams in einem Diagramm
// per Form statt per Farbe unterscheiden; die Akzentfarbe bleibt dadurch
// frei für den Auswahl-Zustand (siehe .is-active in style.css).
// `label` ist die kurze On-Diagramm-Beschriftung (z.B. "DE" für beide
// Defensive Ends); `code` in den data-Attributen bleibt die ausführlichere
// Variante fürs Infofeld (z.B. "DE links").
function positionDot(cx, cy, { id, code, name, desc, label, r = 9, filled = true }) {
  // Farb-/Randwerte wie im Blocker-/Verteidiger-Marker-Referenzentwurf:
  // gefüllt = mittleres Grau mit dunklerem Rand (1px), hohl = nur der
  // dunklere Rand (1.5px), Rasen scheint durch. Kreisform bleibt hier
  // bewusst bestehen (anders als der quadratische Verteidiger-Marker der
  // Referenz), da die Positionen-Seite ein Kürzel mittig im Kreis braucht.
  const fill = filled ? "#888780" : "none";
  const stroke = "#5f5e5a";
  const strokeWidth = filled ? 1 : 1.5;
  const textColor = filled ? "#2b2a26" : "#f5f1e6";
  ...
  return `
    <g class="position-dot" tabindex="0" role="button" data-id="${escapeXml(id)}" data-code="${escapeXml(code)}" data-name="${escapeXml(name)}" data-desc="${escapeXml(desc)}" aria-label="${escapeXml(code)} – ${escapeXml(name)}">
      ...
      <circle class="position-dot-visual" cx="${cx}" cy="${cy}" r="${r}" fill="${fill}" stroke="${stroke}" stroke-width="${strokeWidth}"/>
      <text class="position-dot-label" x="${cx}" y="${cy}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="700" font-family="var(--diagram-font, sans-serif)" fill="${textColor}" pointer-events="none">${escapeXml(displayLabel)}</text>
    </g>`;
}
