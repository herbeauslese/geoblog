...
//
// Beide Teams stehen in einem gemeinsamen Diagramm und werden über Form
// statt Farbe unterschieden (gefüllter vs. hohler Marker, siehe
// positionDot) — die Akzentfarbe bleibt dadurch ausschließlich dem
// Auswahl-Zustand vorbehalten (assets/js/positions.js).
//
// Die Positions-Punkte selbst sind serverseitig/build-time als vollständig
// mit den Detail-Infos (Code/Name/Beschreibung) versehenes SVG gerendert;
// die Interaktion (Hover/Tap/Fokus → Akzentfarbe + Infofeld) übernimmt
// ausschließlich assets/js/positions.js on top.
...
].map((p) => ({ ...p, team: "offense" }));

const DEFENSE_POSITIONS = [
  { id: "de-links", code: "DE links", label: "DE", name: "Defensive End", desc: "Äußerer Lineman, Hauptaufgabe Pass Rush, setzt die Run-Contain-Grenze.", x: -5, y: 1 },
  { id: "dt-1t", code: "DT (1-Technique)", label: "DT", name: "Defensive Tackle", desc: "Innerer Lineman, füllt Run-Gaps, drückt den Pocket von innen.", x: -1, y: 1 },
  { id: "dt-3t", code: "DT (3-Technique)", label: "DT", name: "Defensive Tackle", desc: "Innerer Lineman, füllt Run-Gaps, drückt den Pocket von innen.", x: 2.5, y: 1 },
  { id: "de-rechts", code: "DE rechts", label: "DE", name: "Defensive End", desc: "Äußerer Lineman, Hauptaufgabe Pass Rush, setzt die Run-Contain-Grenze.", x: 5.5, y: 1 },
...
// Free Safety (12) plus ordentlichem Rand an beiden Enden, damit nichts am
// Bildrand klebt.
const CROP = { xMin: -20, xMax: 20, yMin: -8.5, yMax: 14 };
...
function buildCombinedFormationSvg() {
  ...
  for (const pos of ALL_POSITIONS) {
    parts.push(positionDot(sx(pos.x), sy(pos.y), { ...pos, filled: pos.team === "offense" }));
  }
